...
object CrudAuthAction extends BodyParsers {
  ...
  def count[T: Writes](block: RequestWithUser[AnyContent] => T): Action[AnyContent] = {
    fromRequest(OK)(block)
  }
  ...
  def save[E: Writes, T: Format]
    (block: (RequestWithUser[JsValue], T) => Either[E, T])
  : Action[JsValue] = {
  ...
  def delete[E: Writes, T: Writes](block: RequestWithUser[AnyContent] => Either[E, T]): Action[AnyContent] = {
