...
class VoteCounterSpec extends org.specs2.mutable.Specification {
  ...
  "Idea model VoteCounter" should {
    ...
    "retrieve the votes for and idea" in {
      running(FakeApplication(additionalConfiguration = inMemoryDatabase())) {
        val Some(idea) = Idea.findById(1)

        idea must not be none

        idea.votes.pos must equalTo(3)
  ...
  "Comment model VoteCounter" should {
    ...
    "retrieve the votes for a comment" in {
      running(FakeApplication(additionalConfiguration = inMemoryDatabase())) {
        ...
        println(comment.votes.toString)
