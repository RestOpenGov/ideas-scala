...
class SecuredCrudControllerSpec extends Specification {

  def mustReturnAuthError(method: String, path: String, jsonBody: String = "") = {
  ...
  "Controllers using CrudActions (SecuredIdeaTypes)" should {
    ...
    "return an error if no token is passed" in {
